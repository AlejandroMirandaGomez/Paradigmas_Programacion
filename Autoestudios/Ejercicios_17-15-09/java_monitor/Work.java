...
class Lexers{
	...
	static Lexer Eoi(){
		var re_eio = Pattern.compile("(\\s*\\R?)$");
		Lexer lexer = (InputString source) -> {
			var input = source.input();
			var index = source.index();
			
			var matcher = re_eio.matcher(input);
			matcher.region(index, input.length());
			if(!matcher.lookingAt())
				return new Fail<>("No end of file could be match");
			
			var token = new TokenString(TToken.Eoi, "");
			var newSource = new InputString(input, matcher.end());
...
class Parsers {
	...
	@SafeVarargs
	static <I, T, R> Parser <I, T, R> Or_v0(Parser<I, T, R>... parsers){
		Parser<I, T, R> parser = (I source) -> {
			if(parsers.length == 0)
	...
	static <I, T, R> Parser <I, T, R> Or(Parser<I, T, R>... parsers){
		Parser<I, T, R> parser = (I source) -> {
			...
			.map(p -> p.parse(source))
			.filter(r -> r instanceof Ok<I, T, R>)
			.findFirst()
			.orElse(new Fail<>(null));
		};
...
record AST_NodePattern(String id, List<TokenString> labels) {}

///////

class MiniCyphailGrammar {
	static Lexer KeyWord(){
		var re_keyword = Pattern.compile("\\s*(?<keyword>\\w+)");
		...
		Lexer lexer = (InputString source) -> {
			...
			var matcher = re_keyword.matcher(input);
			matcher.region(index, input.length());
			
			if(!matcher.lookingAt())
				return new Fail<>("No keyword could be match");
			
			var keyword = matcher.group("keyword").toLowerCase();
			
			if(!KEYWORDS.contains(keyword))
				return new Fail<>("No keyword could be find");
			
			var token = new TokenString(TToken.Keyword, keyword);
			var newSource = new InputString(input, matcher.end());
	...
	static Parser<InputString, AST_NodePattern, String> NodePattern(){
		Parser <InputString, AST_NodePattern, String> parser = (InputString source) -> {
			...
			List<TokenString> labels = new ArrayList<>();
			...
			var ast = new AST_NodePattern(id, labels);

			return new Ok<>(ast, source);
		};
		return parser;
	}
}
...
void test_8(String title){
	...
	for (var input : inputs) {
		...
		Result<InputString, AST_NodePattern, String> result = nodePattern.parse(source);
		switch (result) {
			case Ok<InputString, AST_NodePattern, String> ok ->
				IO.println(String.format(" ( Ok ) >>> Input = '%s' \n    Tokens = '%s'", input, ok.token));
			case Fail<InputString, AST_NodePattern, String> fail ->
				IO.println(String.format(" (Fail) >>> Input = '%s' \n    Reason = '%s'", input, fail.reason));
		}
	}
}
